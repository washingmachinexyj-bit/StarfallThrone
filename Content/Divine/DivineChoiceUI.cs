...
public sealed class DivineChoiceUI : ModSystem
{
    ...
    {int at=layers.FindIndex(layer=>layer.Name=="Vanilla: Mouse Text");layers.Insert(at<0?layers.Count:at,new LegacyGameInterfaceLayer("StarfallThrone: Divine gift",Draw,InterfaceScaleType.UI));}
    internal static bool Draw()
    {
        if(Tier<0)return true;
        Player player=Main.LocalPlayer;
        if(Main.gameMenu||player.dead||Slot<0||player.inventory[Slot].type!=DivineCatalog.Cache(Tier)){Tier=Slot=-1;return true;}
        ...
        Utils.DrawBorderString(batch,Language.GetTextValue("Mods.StarfallThrone.Divine.Choose"),new Vector2(left+16,top+12),DivineCatalog.Colors[Tier],.9f);
        Utils.DrawBorderString(batch,Language.GetTextValue("Mods.StarfallThrone.Divine.ChooseHint"),new Vector2(left+16,top+42),Color.Silver,.65f);
        ...
        for(int choice=0;choice<4;choice++)
        {
            ...
            Item item=new(DivineCatalog.Weapon(Tier*4+choice));Texture2D tex=TextureAssets.Item[item.type].Value;
            ...
            Utils.DrawBorderString(batch,Language.GetTextValue("Mods.StarfallThrone.Divine.Class"+choice),new Vector2(card.Center.X,card.Y+89),DivineCatalog.Colors[Tier],.62f,.5f);
            if(hover&&click)
            {
                Main.mouseLeftRelease=false;
                if(Main.netMode==NetmodeID.MultiplayerClient)
                {ModPacket packet=ModContent.GetInstance<DivineChoiceUI>().Mod.GetPacket();packet.Write((byte)40);packet.Write((byte)2);packet.Write((byte)Slot);packet.Write((byte)Tier);packet.Write((byte)choice);packet.Send();}
    ...
    public static bool Claim(Player player,int slot,int tier,int choice)
    {
        ...
        Item cache=player.inventory[slot];if(cache.type!=DivineCatalog.Cache(tier)||cache.stack<=0)return false;
        ...
        DivineWorld.Give(player,DivineCatalog.Weapon(tier*4+choice));
